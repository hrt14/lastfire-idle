...
import { RANKS, blend, grade } from "@/lib/bronze";

const STAGE = "bronze" as const;
applyStage(STAGE);
...
for (let step = 0; step < STEPS; step += 1) {
  ...
  if (!seenTech.has(state.bronze.rank)) {
    seenTech.add(state.bronze.rank);
    log.push(`${minute.toFixed(1)}分  🏺 ${RANKS[state.bronze.rank].name}`);
  }
...
console.log(`献上            ${state.bronze.tribute}`);
console.log(`位              ${RANKS[state.bronze.rank].name}`);
console.log(
  `配合            錫 ${Math.round(blend(state) * 100)}%（${grade(state).name}・値 ${grade(state).value}倍）`,
);
console.log(`大鼎            ${state.bronze.casted ? "鋳た" : "まだ"}`);
